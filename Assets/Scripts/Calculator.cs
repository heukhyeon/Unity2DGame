...
public class Calculator : MonoBehaviour
{
    ...
    public RectTransform Lifevalue;
    public Text LifeNotice;
    ...
    {
        float max = Lifebar.sizeDelta.x;
        Vector2 size = Lifevalue.sizeDelta;
        while(size.x<max)
        {
            size.x += 3;
            Lifevalue.sizeDelta = size;
            yield return new WaitForEndOfFrame();
        }
        LifeNotice.gameObject.SetActive(true);
        ProblemCompare();
    }
