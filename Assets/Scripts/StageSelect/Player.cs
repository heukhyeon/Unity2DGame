            ...
            switch(hit.transform.tag)
            {
                ...
                case "App":
                    if(y_dir==1&&!isSelect)
                    {
                        ...
                        y_dir = 0;
                        isSelect = true;
                        string stage = hit.transform.name;
                        if ((stage == "Message" && SmartPhone.memory.Message == StageMemory.Status.NotEnter) || (stage == "Internet" && SmartPhone.memory.Internet == StageMemory.Status.NotEnter))
                            cutUI.CutStart(stage);
                        else SmartPhone.LoadStage(hit.transform.name);
                    }
                    break;
            }
        }
