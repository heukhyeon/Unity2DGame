...
public class ExtendEditor : Editor
{
    ...
    public static void CreateField(ref string content,string name)
    {
    ...
    public static void CreateField(ref int content, string name)
    {
    ...
    /// <param name="content">정수 필드 변동에 반응하는 정수 참조</param>
    /// <param name="name">정수필드 제목</param>
    public static void CreateDelayField(ref int content, string name)
    {
    ...
    public static void ArraySizeModify<T>(ref T[] array, int size)where T:new()
    {
        T[] temp;
    ...
    /// <param name="size">변경할 크기</param>
    public static void ArraySizeModify<T>(List<T> list, int size) where T:new()
    {
    ...
    public static void ArraySizeModify<T>(ref T[] array, int size,ref bool change)
    {
        if (size == array.Length) return;
        change = true;
        T[] temp = new T[size];
    ...
    public static void ArraySizeModify<T>(List<T> list, int size,ref bool change) where T : new()
    {
        if(list.Count!=size)
        {
            change = true;
    ...
    public static void FoldOut(ref bool toggle,string name, Action action)
    {
    ...
    /// <param name="action">활성화시 실행될 함수</param>
    public static void FoldOut(ref bool toggle, string name,int index, Action action)
    {
    ...
    public static void FoldOut<T>(ref bool toggle,string name,Action<T> action,T parameter)
    {
        toggle = EditorGUILayout.Foldout(toggle, name);
        if (toggle)
        {
    ...
    /// <param name="action">활성화시 실행될 함수</param>
    /// <param name="parameter">전송할 매개변수</param>
    public static void FoldOut<T>(ref bool toggle, string name, int index, Action<T> action, T parameter)
    {
        toggle = EditorGUILayout.Foldout(toggle, name + index);
        if (toggle)
        {
